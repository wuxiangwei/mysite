...
写不出娓娓道来的文字，因为缺少耐心，我的行文方式很简单，只需要两步，第一步先列出get到的点，第二步将点连成线，中间写点过度文字。
本来打算和论文那样先一章一节地讨论dmclock的方方面面，后来发现没研究过CPU和内存的调度，直接翻译又觉得浅薄了。



## 从预留说起

起初将Reservation翻译为下限，与上限（Limit）相对，意为保证客户IO不低于下限。“下限”二字很容易让人误解，如果客户自己发送的IO都低于下限那存储系统又如何保证下限。后来，看到有人翻译为预留，顿时觉得很不错。这二字给用户的感觉是，存储系统给客户准备了一部分的IO能力，这部分能力专属于自己不会被别人抢走，当自己没有那么大需求时可以从容应对，当自己超过预留时能够保证得到的最低的服务。但从存储系统来说，并没有专门给你隐藏一部分IO能力，不论是你需求低或者高都是抢来的，低的时候给别人用，高的时候从别人那抢占。
